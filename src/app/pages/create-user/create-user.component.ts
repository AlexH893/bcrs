...
export class CreateUserComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.createUserForm = this.fb.group({
      ...
      securityQuestion1text: [
        this.user.securityQuestions[0].question._id,
        Validators.compose([Validators.required]),
      ],
      securityQuestion2text: [
        this.user.securityQuestions[1].question._id,
        Validators.compose([Validators.required]),
      ],
      securityQuestion3text: [
        this.user.securityQuestions[2].question._id,
        Validators.compose([Validators.required]),
      ],
      securityQuestion1: [
        this.user.securityQuestions[0].answer,
        Validators.compose([Validators.required]),
      ],
      securityQuestion2: [
        this.user.securityQuestions[1].answer,
        Validators.compose([Validators.required]),
      ],
      securityQuestion3: [
        this.user.securityQuestions[2].answer,
        Validators.compose([Validators.required]),
      ],
    });
    this.fetchQuestions();
  ...
  onSubmit(): void {
    ...
    if (this.data.newUser) {
      ...
      this.http.post('/api/users', this.user).subscribe((res: User) => {
