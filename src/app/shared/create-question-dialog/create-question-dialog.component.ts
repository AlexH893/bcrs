...
; Description: create question dialog ts
===========================================
*/
...
export class CreateQuestionDialogComponent implements OnInit {
  ...
  constructor(
    ...
    public data: { question: SecurityQuestion; newQuestion: boolean }
  ) {
    this.securityQuestion = data.question;
    this.titletext = data.newQuestion
  ...
  createQuestion(): void {
    if (this.data.newQuestion) {
      this.http
        .post('/api/security-questions', this.questionForm.value)
      ...
      this.http
        .put(
          `/api/security-questions/${this.securityQuestion._id}`,
